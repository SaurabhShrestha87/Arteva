...
import com.arteva.user.databinding.OnboardingViewBinding
import com.arteva.user.helper.Session
import com.arteva.user.helper.Utils
import com.arteva.user.manager.Utils.changeDpToPx
...
constructor(
    ...
    private val prefManager: Session

    init {
        setUpSlider(binding.root)
        addingButtonsClickListeners()
        prefManager = Session(context)
    ...
    private fun setFirstTimeLaunchToFalse() {
        Session.setFirstTimeFinished(context)
        startSignInUpFrag()
    }
    ...
    private fun navigateToNextSlide() {
        val nextSlidePos: Int = binding.onboardingPageViewPager.currentItem.plus(1) ?: 0
